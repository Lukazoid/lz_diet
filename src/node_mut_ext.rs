...
pub(crate) trait NodeMutExt : NodeMut {
        ...
        where F: FnMut(&mut Self, WalkAction)
    {
        if let Some(mut right) = self.detach_right() {
            right.walk_reshape(|_| WalkAction::Left,
                               move |node| {
    ...
    fn insert_max<F>(&mut self, new_node: <Self as NodeMut>::NodePtr, mut step_out: F)
        where F: FnMut(&mut Self, WalkAction)
    {
        self.walk_reshape(|_| WalkAction::Right,
                               move |node| {
    ...
    fn insert_min<F>(&mut self, new_node: <Self as NodeMut>::NodePtr, mut step_out: F)
        where F: FnMut(&mut Self, WalkAction)
    {
        self.walk_reshape(|_| WalkAction::Left,
                               move |node| {
    ...
    fn walk_reshape_state<S, FI, FS, FO>(&mut self, initial_state: S, mut step_in: FI, stop: FS, mut step_out: FO) -> S
        where FI: FnMut(&mut Self, &mut S) -> WalkAction,
    ...
    fn detach_max<P, F>(&mut self, predicate: P, mut step_out: F) -> Option<<Self as NodeMut>::NodePtr>
        where P: FnOnce(&Self) -> bool, F: FnMut(&mut Self, WalkAction)
    {
    ...
    fn detach_min<P, F>(&mut self, predicate: P, mut step_out: F) -> Option<<Self as NodeMut>::NodePtr>
        where P: FnOnce(&Self) -> bool, F: FnMut(&mut Self, WalkAction)
    {
